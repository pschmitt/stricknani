...
def _build_ai_prompts(
    *,
    ...
    system_prompt = (
        ...
        "IMPORTANT: STRICTLY Minimize redundancy between 'description' and 'steps'.\n"
        "FOR GARNSTUDIO/DROPS PATTERNS:\n"
        "1. The 'description' field MUST contain technical definitions,\n"
        "special techniques, tips, and stitch patterns\n"
        "(e.g., 'KRAUSRIPPEN', 'RAGLANZUNAHMEN', 'STRICKTIPP').\n"
        "2. The 'steps' field MUST contain only the actual chronological\n"
        "knitting instructions (e.g., 'HALSAUSSCHNITT', 'PASSE', 'RUMPFTEIL').\n"
        "3. If a section looks like a tip or technique definition, it goes\n"
        "in 'description'. If it's an instruction on what to do next,\n"
        "it goes in 'steps'.\n"
        "Normalize the text flow: fix broken line breaks and sentences that are "
        "split across multiple lines incorrectly. "
        "You should restructure and rephrase the contents to improve clarity "
        "and apply Markdown markup for increased readability, "
        "as long as all factual information and the original meaning are preserved.\n"
