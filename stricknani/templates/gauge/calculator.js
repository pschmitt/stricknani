document.addEventListener("DOMContentLoaded", () => {
	...
	patternStitches?.addEventListener("input", () =>
		mirrorIfEmpty(patternStitches, userStitches),
	);
	patternRows?.addEventListener("input", () =>
		mirrorIfEmpty(patternRows, userRows),
	);
...
document.body.addEventListener("htmx:afterRequest", function (event) {
	if (event.detail.target.id === "result") {
		try {
			...
			const adjustedRows =
				data.adjusted_rows === null
					? "{{ _('N/A') }}"
					: data.adjusted_rows;
			const patternRows =
				data.pattern_row_count === null
					? ""
					: ` {{ _('and') }} ${data.pattern_row_count} {{ _('rows') }}`;
			const resultHtml = `
