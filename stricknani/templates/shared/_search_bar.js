        (function () {
            ...
            const suggestionContainer = document.getElementById('{{ search_id }}-suggestions');
            const suggestionList = document.getElementById('{{ search_id }}-suggestion-list');
	            const clearButton = document.querySelector('#{{ wrapper_id }} [data-search-clear]');
	            let currentPrefix = '';
	            let activeIndex = -1;
	            let lastSuggestions = [];
	            let lastMatch = null;
            ...
            const clearActiveState = () => {
                activeIndex = -1;
                suggestionList.querySelectorAll('[data-suggestion-index]').forEach((item) => {
            ...
            searchInput.addEventListener('input', async (e) => {
                ...
                if (match) {
                    ...
                    const baseUrl = '{{ search_url }}'.replace(/\/+$/, '');
	                    ...
	                    try {
	                        const response = await fetch(`${baseUrl}/search-suggestions?type=${type}&q=${encodeURIComponent(q)}`);
                        ...
                        if (suggestions.length > 0) {
                            ...
                            const matchedExact = suggestions.some((s) => {
                                const formattedValue = formatSuggestionValue(match, s).toLowerCase();
                            ...
                            if (matchedExact) {
	                            ...
	                            suggestionList.innerHTML = suggestions.map((s, index) => {
	                                return `
	                                <li>
            ...
            // Close suggestions on click outside
	            ...
	            suggestionList.addEventListener('click', (e) => {
	                ...
	                const index = Number.parseInt(button.getAttribute('data-suggestion-index') || '0', 10);
	                ...
	                window.selectSearchSuggestion('{{ search_id }}', formattedValue);
	            });
