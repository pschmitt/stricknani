    ...
    async function deleteAttachment(idOrToken) {
        ...
        if (typeof idOrToken === 'number' || (typeof idOrToken === 'string' && idOrToken.length !== 32)) {
            ...
            const filename = attachmentEl?.dataset.attachmentName || '{{ _("Attachment") }}';
    ...
    async function deleteImage(imageId) {
        ...
        const filename = imgTag?.alt || linkTag?.dataset.pswpCaption || '{{ _("Image") }}';
    ...
    async function saveStep(button) {
        ...
        button.innerHTML = '<span class="mdi mdi-loading mdi-spin"></span> {{ _("Saving...") }}';
    ...
    try {
        ...
        console.error('Import failed:', error);
        window.showToast?.(error.message || '{{ _("Failed to import pattern") }}', 'error');
